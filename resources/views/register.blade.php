...
<body class="bg-gray-100 flex items-center justify-center h-screen">
    <div class="w-full max-w-md bg-white p-8 rounded-lg shadow-md">
        ...
        <form method="POST" action="{{ route('register.register-store') }}">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="password_confirmation">Confirm Password <span class="text-red-500">*</span></label>
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Role <span class="text-red-500">*</span></label>
                <select required name="role" class="w-full mb-4 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option disabled selected>-- Select Role --</option>
                    <option value="1" {{ old('role') && old('role') == 1 ? 'selected' : '' }}>
                        Staff
                    </option>
                    <option value="2" {{ old('role') && old('role') == 2 ? 'selected' : '' }}>
                        Supervisor
                    </option>
                    <option value="3" {{ old('role') && old('role') == 3 ? 'selected' : '' }}>
                        Manager
                    </option>
                    <option value="4" {{ old('role') && old('role') == 4 ? 'selected' : '' }}>
                        Executive
                    </option>
                </select>
            </div>
